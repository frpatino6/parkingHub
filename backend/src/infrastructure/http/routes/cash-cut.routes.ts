...
export function createCashCutRoutes(controller: CashCutController): Router {
  ...
  router.post('/open', requireRole(UserRole.OPERATOR), controller.open);
  router.post('/close', requireRole(UserRole.OPERATOR), validate(closeSchema), controller.close);
  
  // Manual Movements
  router.post('/movements', requireRole(UserRole.OPERATOR), validate(movementSchema), controller.addMovement);
